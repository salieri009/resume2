...
export const L10N: Record<Exclude<Lang, 'en'>, Record<ProjectKey, L10NProjectOverride>> = {
  ko: {
    crowd: {
      summary: '이동이 불편한 여행자를 위한 실시간 혼잡·근접 경보입니다. 딥러닝 수업 마감 뒤에도 계속 만들었습니다.',
      role: '팀 리드 — YOLO 학습·추론·알림 담당 (커밋의 97%)',
      outcome: 'v2.6.0 릴리스, 라이브 데모 운영 중',
      overview:
        'UTS 딥러닝(42028)에서 시작했습니다. YOLOv8로 사람을 감지하고, 바운딩박스 근접 휴리스틱으로 다가오는 인파를 경고합니다. JRDB로 파인튜닝했고, React·Spring Boot·FastAPI를 Docker로 묶었습니다. 학습은 SageMaker, 데이터는 DVC입니다.',
      roleDetail:
        'ML을 맡았고 코드 대부분도 제가 썼습니다. 199개 커밋 중 193개. 학습 파이프라인, FastAPI 추론·알림, SageMaker 스크립트, v2.5.0~v2.6.0 릴리스(스펙·ADR·학습 리포트 포함).',
      problems: [
        {
        ...
        },
        {
          ...
          s: '학습을 AWS SageMaker(ml.g5.xlarge)로 옮기고 스크립트 런처를 만들었습니다. 모든 런을 재현·재개 가능하게 했습니다.',
        },
      ],
      period: '2026년 1학기',
      teamSize: '3명 — ML 리드',
      decisions: [
        {
          ...
          why: '비동기 I/O에 모델 옆의 파이썬 런타임. 프레임당 지연이 낮고 YOLO 스택이 깔끔하게 붙습니다.',
        },
        {
          choice: '추론을 직접 호출하지 않고 Spring Boot 게이트웨이를 둠',
          why: '인증, 레이트 리밋, API 계약을 한 곳에 모읍니다. 추론 컨테이너는 언제든 교체할 수 있습니다.',
        },
        {
      ...
      lessons: [
        '스펙, ADR, 학습 리포트가 ML 작업을 리뷰 가능하게 만듭니다. 돌아가는 것만으로는 부족합니다.',
        '클라우드 학습의 가치는 속도보다 재현성에 있습니다.',
      ],
      future: ['엣지 디바이스용 모델 양자화', 'WebSocket 프레임 스트리밍', '오토스케일링 붙인 k8s 배포'],
    },
    iotbay: {
      summary: 'IoT 이커머스 풀스택입니다. 카탈로그, 결제, 주문, 관리자 도구. 8인 팀이 배포할 때 필요한 규율로 짰습니다.',
      role: '팀 기여자 — 8인 팀, 총 297 커밋',
      outcome: 'E2E 테스트 118개 통과 · Docker/GHCR로 CI/CD',
      overview:
        'UTS 소프트웨어 개발 입문 팀 과제입니다. IoT 기기 이커머스. 회원가입, 역할별 접근, 상품 검색, 장바구니, 결제, 배송 주문, KPI 대시보드와 CSV/JSON보내기. Servlet·Service·DAO 레이어드 MVC, SQLite.',
      roleDetail:
        '소스 파일 111개 코드베이스에서 8명 중 한 명으로 주문 라이프사이클과 테스트 스위트를 맡았습니다. 팀은 10개 기능 영역을 덮는 Selenium E2E 118개와 보안 경계 테스트를 만들었고, 푸시마다 GitHub Actions가 전부 돕니다. 배포는 Docker 이미지로 GHCR에 올라갑니다.',
      problems: [
        {
          p: '8명이 한 코드베이스에서 작업하다 보니 서로의 기능을 계속 깨뜨렸습니다.',
          s: 'Servlet → Service → DAO로 계층을 나누고 엔티티마다 인터페이스를 뒀습니다. 푸시마다 118개 테스트를 돌리는 CI를 붙이니, 통합 사고가 런타임 버그가 아니라 빨간 빌드로 잡혔습니다.',
        },
        {
          p: '보안 리뷰에서 CSRF, SQL 인젝션, XSS가 지적됐습니다.',
          s: 'CSRF 토큰 검증, prepared statement, 출력 인코딩, 인증된 행동의 접근 로그를 넣었습니다. 그 상태를 유지하려고 보안 경계 테스트 14개를 남겼습니다.',
        },
      ],
      period: '2025년 1학기',
      teamSize: '8명',
      decisions: [
        ...
        },
        {
          ...
          why: '노트북 8대와 CI에서 설정이 필요 없습니다. DAO 계층 덕분에 나중에 PostgreSQL로 바꾸는 범위도 갇혀 있습니다.',
        },
        {
      ...
      lessons: [
        '푸시마다 전부 돌리는 CI는 통합 사고를 런타임 버그가 아니라 빨간 빌드로 바꿉니다.',
    ...
    farm: {
      ...
      overview:
        'UTS 그래픽스 과제를 브리프보다 멀리 끌고 갔습니다. 클릭으로 동물·건물·소품을 그리드에 배치하고 지형을 넓힙니다. 사계절, 번개 폭풍 포함 다섯 가지 날씨, 달이 도는 밤 모드, OpenWeatherMap 연동 선택.',
      roleDetail:
        '렌더링을 맡았습니다. 씬 아키텍처, 계절·날씨 파티클, 낮/밤 조명, 텍스처 조합이 아니라 GLSL로 직접 쓴 겨울 오로라. v2 저장소는 원래 제출물을 씬 구성부터 갈아엎은 리팩터링입니다.',
      problems: [
        {
          p: '꽃잎, 비, 눈, 반딧불 — 수천 개의 파티클이 프레임레이트를 무너뜨렸습니다.',
          s: '이펙트를 인스턴스드 렌더링과 GPU 파티클로 옮기고 프레임당 시뮬레이션에 상한을 뒀습니다. 폭풍 속에서도 씬이 인터랙티브하게 유지됩니다.',
        },
        {
          p: '텍스처로 만든 오로라는 평면적이고 가짜처럼 보였습니다.',
          s: 'GLSL 프래그먼트 셰이더를 직접 썼습니다. 시간 유니폼으로 움직이는 레이어드 노이즈와 색 밴드로, 드로우콜 하나에서 볼륨감 있는 북극광을 만들었습니다.',
        },
      ],
      ...
      decisions: [
        ...
        },
        {
          ...
          why: '셰이더 반복에 빠른 HMR. 씬 그래프 오용은 타입이 초기에 잡아줬습니다.',
        },
      ],
      tradeoffs: [
        {
      ...
      lessons: [
        '성능 작업이 곧 디자인 작업입니다. 인스턴싱이 가능한 이펙트의 범위를 바꿨습니다.',
        '씬 구성이 빚이 됐다면 재작성(v2)은 값을 합니다.',
      ],
      future: ['낮/밤 사이클 보간', '농장 배치 저장/불러오기', '모바일 터치 컨트롤'],
    },
    gundam: {
      summary: '인증이 붙은 풀스택 CRUD 게시판. 서버리스 스택을 배우려고 여름에 혼자 만들었습니다.',
      role: '단독 개발 — 스키마부터 배포까지',
      outcome: '서버리스 백엔드에서 OAuth + JWT 인증 동작',
      overview:
        '2025년 여름방학 프로젝트. 규칙은 하나였습니다. 어려운 부분에서 지름길 없음. Google OAuth와 JWT 세션이 붙은 CRUD 게시판을 Next.js 프론트와 AWS Chalice 위 Python 서버리스 백엔드 + PostgreSQL로 만들었습니다. 한 번도 배포해 본 적 없는 기술만 골랐습니다.',
      roleDetail:
        '전부 했습니다. 데이터베이스 스키마, Chalice API 라우트와 미들웨어, OAuth와 JWT 발급/검증, Next.js 프론트엔드, 배포. 혼자 하는 프로젝트는 팀 컨벤션이 없을 때 코드를 어떻게 짜는지 시험하는 자리입니다.',
      problems: [
        {
          p: '상태가 없는 서버리스 백엔드에서의 세션 처리.',
          s: 'Google 신원을 서버에서 검증한 뒤 짧은 수명의 JWT를 발급하고, 매 요청마다 Chalice 미들웨어에서 확인합니다. 서버 쪽 세션 저장소가 필요 없어졌습니다.',
        },
        {
          p: 'Next.js를 Python 서버리스 API에 붙이는 건 처음이었습니다.',
          s: '얇은 타입드 API 클라이언트를 만들어 계약을 한 곳에 모았습니다. 백엔드를 리팩터링해도 UI로 새지 않습니다.',
        },
      ],
      period: '2025년 여름방학',
      teamSize: '단독',
      ...
      tradeoffs: [
        {
          rejected: 'DynamoDB',
          why: '게시판 도메인(글·댓글·사용자)은 관계형입니다. 키-밸류보다 PostgreSQL이 맞았습니다.',
        },
        {
          rejected: 'NextAuth 올인원',
          why: 'OAuth 검증과 JWT 발급 경로를 직접 만드는 게 목적이었습니다. 인증 플로우를 한 번은 직접 가져가고 싶었습니다.',
        },
      ],
      lessons: [
    ...
    ephemeral: {
      summary: '시간을 유체로 다루는 제너러티브 아트. 주의가 흐름을 바꿉니다. 스케치가 아니라 작은 프로덕트처럼 짰습니다.',
      role: '단독 — 컨셉, 시뮬레이션, 아키텍처',
      outcome: '실시간 유체 + 오디오 합성, 에셋 제로',
      overview:
        '시간을 "주의력의 저수지"로 시각화한 인터랙티브 아트입니다. 캔버스를 가만두면 시간이 느리게 흐르고, 마우스를 빠르게 휘저으면 난류가 유체와 채도, 소리까지 흐트러뜨립니다. 매시간 태양 방울이 지나가고, 15분마다 사이매틱 파문이 울립니다. 오디오는 전부 실시간 합성이고, 미디어 에셋은 없습니다.',
      roleDetail:
        '혼자 만들었고, 구조는 일부러 무겁게 잡았습니다. 전역 변수 없는 IoC, GC 멈춤 50–70% 줄인 오브젝트 풀, 교체 가능한 렌더러, 정적 배경을 다시 그리지 않는 3계층 파이프라인.',
      problems: [
        {
          p: '수천 개의 잉크 파티클이 GC 멈춤을 일으켜 페이싱을 깨뜨렸습니다.',
