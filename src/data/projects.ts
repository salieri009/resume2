...
export const L10N: Record<Exclude<Lang, 'en'>, Record<ProjectKey, L10NProjectOverride>> = {
  ko: {
    crowd: {
      summary: '이동이 불편한 여행자를 위한 실시간 혼잡·근접 경보입니다. 딥러닝 수업이 끝난 뒤에도 계속 만들었습니다.',
      role: '팀 리드 — YOLO 학습·추론·알림 담당 (커밋의 97%)',
      outcome: 'v2.6.0 릴리스, 라이브 데모 운영 중',
      overview:
        'UTS 딥러닝(42028)에서 시작했습니다. YOLOv8로 사람을 감지하고, 바운딩박스 근접 휴리스틱으로 다가오는 인파를 경고합니다. JRDB로 파인튜닝했고, React·Spring Boot·FastAPI를 Docker로 묶었습니다. 학습은 SageMaker, 데이터는 DVC.',
      roleDetail:
        'ML을 맡았고 코드 대부분도 제가 썼습니다 — 199개 커밋 중 193개. 학습 파이프라인, FastAPI 추론·알림, SageMaker 스크립트, v2.5.0~v2.6.0 릴리스(스펙·ADR·학습 리포트 포함)까지.',
      problems: [
        {
      ...
      decisions: [
        {
          ...
          why: '비동기 I/O에, 모델 바로 옆에 붙는 파이썬 런타임. 프레임당 지연이 낮고 YOLO 스택이 깔끔하게 붙습니다.',
        },
        {
      ...
      tradeoffs: [
        ...
        },
        {
          ...
          why: '웹캠 프레임레이트에서는 프레임당 HTTP로도 충분했고 디버깅도 쉬웠습니다. 스트리밍은 백로그로 미뤘습니다.',
        },
      ],
      lessons: [
    ...
    iotbay: {
      summary: 'IoT 이커머스 풀스택입니다. 카탈로그, 결제, 주문, 관리자 도구까지. 8인 팀이 배포할 때 필요한 규율로 짰습니다.',
      ...
      problems: [
        {
          ...
          s: 'Servlet → Service → DAO로 계층을 나누고 엔티티마다 인터페이스를 뒀습니다. 푸시마다 118개 테스트를 돌리는 CI를 붙이니 통합 사고가 런타임 버그 대신 빨간 빌드로 잡혔습니다.',
        },
        {
      ...
      decisions: [
        ...
        },
        {
          ...
          why: '노트북 8대와 CI에서 설정이 필요 없습니다. DAO 계층 덕분에 나중에 PostgreSQL로 바꿀 때도 범위가 갇혀 있습니다.',
        },
        {
      ...
      lessons: [
        '푸시마다 전부 돌리는 CI는 통합 사고를 런타임 버그 대신 빨간 빌드로 바꿉니다.',
    ...
    farm: {
      ...
      overview:
        'UTS 그래픽스 과제를 브리프보다 멀리 끌고 갔습니다. 클릭으로 동물·건물·소품을 그리드에 배치하고 지형을 넓힙니다. 사계절, 번개 폭풍을 포함한 다섯 가지 날씨, 달이 도는 밤 모드, OpenWeatherMap 연동은 옵션.',
      roleDetail:
        '렌더링을 맡았습니다. 씬 아키텍처, 계절·날씨 파티클, 낮/밤 조명, 텍스처 조합이 아니라 GLSL로 직접 쓴 겨울 오로라까지. v2 저장소는 원래 제출물을 씬 구성부터 갈아엎은 리팩터링입니다.',
      problems: [
        {
          p: '꽃잎, 비, 눈, 반딧불 — 수천 개의 파티클이 프레임레이트를 무너뜨렸습니다.',
          s: '이펙트를 인스턴스드 렌더링과 GPU 파티클로 옮기고 프레임당 시뮬레이션에 상한을 뒀습니다. 폭풍 속에서도 씬은 인터랙티브하게 유지됩니다.',
        },
        {
    ...
    gundam: {
      ...
      overview:
        '2025년 여름방학 프로젝트. 규칙은 하나, 어려운 부분에서 지름길 없기. Google OAuth와 JWT 세션이 붙은 CRUD 게시판을 Next.js 프론트와 AWS Chalice 위 Python 서버리스 백엔드 + PostgreSQL로 만들었습니다. 한 번도 배포해 본 적 없는 기술만 골랐습니다.',
      roleDetail:
        '전부 했습니다. 데이터베이스 스키마, Chalice API 라우트와 미들웨어, OAuth와 JWT 발급/검증, Next.js 프론트엔드, 배포까지. 혼자 하는 프로젝트는 팀 컨벤션이 없을 때 코드를 어떻게 짜는지 시험하는 자리입니다.',
      problems: [
        {
      ...
      decisions: [
        {
          ...
          why: '혼자 하는 프로젝트에 서버 관리는 제로. IAM/API Gateway를 문서 대신 배포로 배우고 싶었습니다.',
        },
        {
    ...
    ephemeral: {
      ...
      overview:
        '시간을 "주의력의 저수지"로 시각화한 인터랙티브 아트입니다. 캔버스를 가만두면 시간이 느리게 흐르고, 마우스를 빠르게 휘저으면 난류가 유체와 채도, 소리까지 흐트러뜨립니다. 매시간 태양 방울이 지나가고, 15분마다 사이매틱 파문이 울립니다. 오디오는 전부 실시간 합성, 미디어 에셋은 없습니다.',
      roleDetail:
        '혼자 만들었고, 구조는 일부러 무겁게 잡았습니다. 전역 변수 없는 IoC, GC 멈춤을 50–70% 줄인 오브젝트 풀, 교체 가능한 렌더러, 정적 배경을 다시 그리지 않는 3계층 파이프라인까지.',
      problems: [
        {
          p: '수천 개의 잉크 파티클이 GC 멈춤을 일으켜 페이싱을 깨뜨렸습니다.',
          s: '프레임마다 할당하던 걸 사전 할당된 오브젝트 풀과 팩토리로 바꿨습니다. GC 멈춤이 50–70% 줄었고, 난류 속에서도 유체는 매끄럽습니다.',
        },
        {
      ...
      decisions: [
        {
          ...
          why: 'GC 멈춤이 페이싱을 깨뜨렸습니다. 사전 할당으로 50–70% 줄이고 흐름을 지켰습니다.',
        },
        {
  ...
  ja: {
    ...
    gundam: {
      ...
      overview:
        '2025年夏休みのプロジェクト。ルールは一つ、難しい部分でショートカットしないこと。Google OAuthとJWTセッション付きのCRUD掲示板を、Next.jsフロントとAWS Chalice上のPythonサーバーレスバックエンド + PostgreSQLで構築。出荷したことのない技術だけでレイヤーを選びました。',
      roleDetail:
