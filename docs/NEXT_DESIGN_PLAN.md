...
## v4 계획 — 이력서 2장 규격 + 디자인 폴리시 (2026-07-16 승인, 즉시 실행)
...
**폴리시 항목(같은 착지):**
1. `.sal-glow-bg` 제거 — v1 계획이 bento 그라디언트와 함께 "가장 덜 건축적인 제스처"로 지목했는데 그라디언트만 제거되고 글로우가 남아 있었다.
2. 마지막 미토큰화 transition(`.sal-root`의 `0.3s ease`) 토큰화 — 이걸로 전이 어휘 통일 100%.
3. 이력서 2장 밀도 검증: `?sheets=r` 프리뷰에서 R-001/R-002 각각의 높이가 A4 본문 근사(≤990px)에 들어가는지 en/ko/ja 실측.
