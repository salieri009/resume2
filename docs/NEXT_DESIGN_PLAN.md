...
1. **이력서 CTA.** ~~`RESUME_PDF.available === false`라 히어로에 버튼이 하나뿐.~~ **결정(2026-07-19): 인쇄 방식 유지.** 이력서는 PrintSet이 R-001/R-002로 생성하며 PDF 파일은 두지 않는다. 크롬의 `DOC` 버튼은 `RÉSUMÉ`로 개명해 발견 가능성 해소, `?sheets` / `?sheets=r` 온스크린 라우트 구현으로 인쇄물 푸터 안내도 실동작.
